...
public class TimeTableServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		isTrue = TimeTableDBUtil.inserttimetable(moduleCode,module,topic,date,time,lecturer,platform,description);

		if(isTrue == true) {
			RequestDispatcher dis = request.getRequestDispatcher("SMsuccess.jsp");
			dis.forward(request, response);
		}else {
			RequestDispatcher dis2 = request.getRequestDispatcher("SMunsuccess.jsp");
